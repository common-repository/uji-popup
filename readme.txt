=== Uji Popup ===
Contributors: ujimoto
Tags: onclick popup, optin popup, pop-up, pop-ups, popup, popup ads, popup advertising, popup box, popup form, popup message, popup window, popups, modal, modals, unblockable, wordpress popup, auto open, automatic popup, conversion, conversions, custom popup, dialog box, exit intent, fancy box, fancy popup, form modal, post popup, page popup, screen popup, marketing
Requires at least: 4.0
Tested up to: 4.8.2
Stable tag: 1.4.3

Uji Popup - Create your custom popup.

== Description ==

Uji Popup is a WordPress popup plugin which allown you to convert visitors on your site/blog into taking an action whether its advertise, subscribing into your newsletter, offering a discount or cupon. The popup messages are displayed as soon as the visitors opens the page. Shows dynamically when a visitor goes to close your website.

= Features: =

* NEW: Exit-Intent technology has been one of the most successful Internet marketing techniques
* Popups are shown when the page is loaded
* Use image ads or text ads (Flash, HTML, Images, Videos, Audio, Embed Code etc.)
* Timer(countdown) can be enabled/disabled
* Close/Skip button can be enabled/disabled
* Impression: count number of displays
* Wait timer. Enter how long the popup should wait until will be shown.
* Page control allows you to set on all pages/homepage or just on custom pages
* Multi language Backend/Frontend support
* Support customize colors
* Support for cache plugins (WP Super Cache and W3 or W3 Total Cache)

<a href="http://wpmanage.com/uji-popup">Uji Popup Premium</a> is now available with new features.

http://www.youtube.com/watch?v=BkdNGiryUIw


**Plugin's Official Site**
WPmanage [(http://www.wpmanage.com/uji-popup/)](http://www.wpmanage.com/uji-popup/)

== Installation ==

1. Upload the `uji-popup` folder to the `/wp-content/plugins/` directory
2. Activate the Uji Popup plugin through the 'Plugins' menu in WordPress
3. Create your new Popup style in Settings/Popup
4. Place your Popup in Post/Page via Edit: Post or Page

== Screenshots ==

1. Create your new Popup content.
2. Popup Options.
3. Front-end view.


* SUPPORT *
Contact us for support: info@wpmanage.com

== Changelog ==

= 1.4.3 =

* Fixed multiple popups issue to save the popup options.
* Other improvements and bug fixes

= 1.4.2 =

* Added: UI Improvements
* Fixed a bug for error displayed on some themes

= 1.4.1 =

* Fixed a bug that could cause issue in theme when use WooCommerce
* Fixed strong tag close missed in the previous version

= 1.4 =

* Added: New Page/Post selection
* Other fixes and improvements added

= 1.3 =

* Added: Exit-intent technology
* Added: Option allows the user to close the modal by clicking the overlay (outside)
* Fix: minor admin style fixes 

= 1.2 =

* Added: New modern style popup
* Bug Fix: Minimum width size

= 1.1.2 =

* Fix: Excerpt issue on some themes
* Fix: Bootstrap overwrite on some themes

= 1.1.1 =

* New: Location: Choose where the popup will be positioned
* Bug Fix: Close button overflow on some themes

= 1.1.0 =

* NEW: Add popup by link or button
* Compatibility with WP 4.0 beta 2
* New Countdown Module
* Fix Colorpicker
* Fix add URL issue

= 1.0.3 =

* Fix IE issue.

= 1.0.2 =

* Fix Popup Options bug

= 1.0.1 =

* Few minor fixes.

= 1.0 =

* First version. Stable version.