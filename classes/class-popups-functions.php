...
class Uji_Popups_Functions {
    ...
    protected function is_popups( $id, $ajax = null ) {
        ...
        while ( $queryin->have_posts() && $cicle ):
            ...
            if ( $valid && $where == 'show_cust' && !is_home() && !is_front_page() ) {
                if ( !$this->get_post_meta_type(get_the_ID(), $id, 'post') ) {
                        $valid = false;
                }
            }
    ...
    /**
     * Get Post Meta Post/Tax
     * @since  1.4
     */
    protected function get_post_meta_type( $pid, $id, $type ){
        //Check Post/Pages
        if($type == 'post'){
            $pages = get_post_meta( $pid, 'pop_posts', true );
            $p_type = get_post_type( $id );
            
            return ( empty($pages) || ( !empty($pages) && is_array($pages[$p_type]) && in_array( $id, $pages[$p_type] ) ) ) ? true : false;
        }
    }
